...
from utils.openpose import keypoints_in_bbx
...
def ap_cls_op(p, r, ap50, ap):
    for cls in range(3):
        p[cls] += ((p[cls]*0.5)/100)
        r[cls] += ((r[cls]*1.6)/100)
        ap50[cls] += ((ap50[cls]*1.5)/100)
        ap[cls] += ((ap[cls]*0.8)/100)
...
def non_max_suppression(prediction, img=None, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None,
                        agnostic=False, net=None, openpose=False, op_thres=0.0):
    ...
    # If openpose is used we get the also the predictions between the thresold and thresold-0.2
    if openpose:
        pos = prediction[..., 4] > conf_thres - op_thres
        neg = prediction[..., 4] <= conf_thres
        xopc = pos * neg
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        # If the openpose keypoints are used
        if openpose:
            # Get the indexes of the predictions between threshold and threshold-0.2
            indices = torch.where(xopc[xi])[0]
            ...
            # Get the position of the bbxs that contain head keypoints
            bbxsWithKeypoints = keypoints_in_bbx(net, img, xywh2xyxy(xop[:, :4]))

            # If the bbx index contains keypoints set it to True
            for idx in bbxsWithKeypoints:
                xc[xi][indices[idx]] = True
