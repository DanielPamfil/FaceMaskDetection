...
def get_valid_pairs(output, detected_keypoints, frameWidth, frameHeight):
    valid_pairs = []
    invalid_pairs = []
    n_interp_samples = 10
    paf_score_th = 0.1
...
def keypoints_in_bbx(net, img, bbxs, threshold=0.1, device="cpu"):
    ...
    for part in range(nPoints):
        ...
        for i in range(len(keypoints)):
            for idx, bbx in enumerate(bbxs):
                if part in [0, 14, 15, 16, 17] and point_in_bbx(bbx, keypoints[i][0:2]):
                    indexList.append(idx)
    """
    numberOfTargets = 0
    for part in [0, 14, 15, 16, 17]:
        probMap = output[0, part, :, :]
        probMap = cv2.resize(probMap, (img.shape[1], img.shape[0]))
        keypoints = get_keypoints(probMap, threshold)
        # print("Keypoints - {} : {}".format(keypointsMapping[part], keypoints))
        for i in range(len(keypoints)):
            for idx, bbx in enumerate(bbxs):
                if point_in_bbx(bbx, keypoints[i][0:2]):
                    indexList.append(idx)
